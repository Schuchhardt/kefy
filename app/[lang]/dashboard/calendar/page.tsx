...
export default function CalendarPage() {
  ...
  const grouped = posts.reduce<Record<string, ScheduledPost[]>>((acc, post) => {
    const key = post.scheduled_at
      ? new Date(post.scheduled_at).toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })
      : 'Sin fecha';
    if (!acc[key]) acc[key] = [];
    acc[key].push(post);
    return acc;
  ...
  return (
    <div style={{ padding: '40px 48px', maxWidth: 880 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 28 }}>
        <div>
          <h1 style={{ fontFamily: 'var(--font-syne)', fontSize: 26, fontWeight: 700 }}>Calendario</h1>
          <p style={{ color: 'var(--muted)', fontSize: 14, marginTop: 4 }}>Programa y gestiona tus publicaciones en redes sociales</p>
        </div>
        <button
          onClick={openForm}
          style={{
      ...
      {showForm && (
        <form onSubmit={handleSchedule} style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
            <div>
              <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>CONTENIDO</label>
            ...
            <div>
              <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>CUENTA</label>
          ...
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>FECHA Y HORA</label>
            <input
              type="datetime-local"
              style={inputStyle}
      ...
      {!loading && accounts.length === 0 && (
        <div style={{
          ...
          Ve a <a href="settings" style={{ color: 'var(--accent)' }}>Ajustes</a> para conectar tus redes sociales.
        </div>
      )}
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '60px 0' }}>
          <p style={{ color: 'var(--muted)', fontSize: 15 }}>No hay publicaciones programadas</p>
        </div>
      ) : (
                ...
                <div key={post.id} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 13, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: 2 }}>
                      {post.kefy_content_items?.body?.slice(0, 80) ?? post.kefy_content_items?.title ?? '(sin texto)'}
                    </p>
                    <p style={{ fontSize: 12, color: 'var(--muted)' }}>
                      {post.kefy_social_accounts?.username ?? 'Cuenta desconocida'}
                    </p>
                    {post.error_message && (
